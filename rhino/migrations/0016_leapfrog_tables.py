...
class Migration(SchemaMigration):
    ...
    rhino_class_names = ('media', 'person', 'account', 'object', 'userstream', 'userreplystream', 'usersetting')

    def forwards(self, orm):
        for name in self.rhino_class_names:
            db.rename_table('rhino_%s' % name, 'leapfrog_%s' % name)


    def backwards(self, orm):
        for name in reversed(self.rhino_class_names):
            db.rename_table('leapfrog_%s' % name, 'rhino_%s' % name)
    ...
    models = {
        ...
        'rhino.account': {
            'Meta': {'unique_together': "(('service', 'ident'),)", 'object_name': 'Account', 'db_table': "'leapfrog_account'"},
        ...
        'rhino.media': {
            'Meta': {'object_name': 'Media', 'db_table': "'leapfrog_media'"},
        ...
        'rhino.object': {
            'Meta': {'unique_together': "(('service', 'foreign_id'),)", 'object_name': 'Object', 'db_table': "'leapfrog_object'"},
        ...
        'rhino.person': {
            'Meta': {'object_name': 'Person', 'db_table': "'leapfrog_person'"},
        ...
        'rhino.userreplystream': {
            'Meta': {'unique_together': "(('user', 'reply'),)", 'object_name': 'UserReplyStream', 'db_table': "'leapfrog_userreplystream'"},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'rhino.usersetting': {
            'Meta': {'unique_together': "(('user', 'key'),)", 'object_name': 'UserSetting', 'db_table': "'leapfrog_usersetting'"},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'rhino.userstream': {
            'Meta': {'unique_together': "(('user', 'obj'),)", 'object_name': 'UserStream', 'db_table': "'leapfrog_userstream'"},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
